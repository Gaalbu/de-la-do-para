...
| C23 — UI administrativa de produtos | `specs/SPEC-catalog.md` §§7–9, API C22 | `product-admin.component.spec.ts`; Playwright `admin-products.spec.ts`; `scripts/verify.sh frontend` | local `feat/c23-product-admin-ui`, baseada em C22 | Alimentos/artesanato, produtores ativos, variantes, medidas, peso, fragilidade, erro recuperável e confirmação de desativação; 10 testes Angular e 3 Playwright verdes; E2E usa API simulada; commit/PR pendentes |
